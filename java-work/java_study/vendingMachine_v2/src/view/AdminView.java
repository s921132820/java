...
//        Product product = new Product();
        ...
        price = sc.nextInt();
        System.out.println("재고 등록(채울 갯수)");
        stock = sc.nextInt();
...
//        product.setStock(stock);
    ...
    public void viewAllList() {
        ...
        printHeader();
        // 출력
        // 1. service에 전체리스트 요청 -- List <Product>타입으로 받아야함, 이 타입으로 등록했기 때문에
        productList = adminService.getAllList();
        // 2. 상품 리스트가 비어있을 경우 출력
        if (productList.size() == 0) {
            System.out.println("현재 자판기의 상품이 없습니다.");
        } else {
            // 3. 받은 리스트를 출력
